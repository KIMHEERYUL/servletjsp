...
public class ContentController extends HttpServlet{
	...
	//클라이언트에서 최초로 서블릿을 요청했을 때 딱 한번만 실행(load-on-startup 옵션이 없을 경우)
	//load-on-startup이 있을경우 Deployment시에 딱 한번 실행
	//역할: service(), doPost(), doGet()에서 사용하는 데이터를 초기화해주는 역할 (문자셋, 기본경로 등등)
	@Override
	...
	//클라이언트가 요청할때마다 실행
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			System.out.println("paraml : " + param1);
			request.getRequestDispatcher("/WEB-INF/views/exam03/content.jsp").forward(request, response);
		}
	//서블릿이 제거될때(undeployment)
	@Override
	public void destroy() {
